...
- 소개(About): 기능 소개 카드 8개(AI 챗봇/정책 검색/신청 관리/맞춤 추천/커뮤니티/정책 제안/AI 검토/팀모집 순)를 좌우 스와이프 캐러셀로 탐색(양옆 화살표·하단 점 인디케이터) + 상단에 탭 바로 바로 이동 가능. 단, 팀모집은 정책 제안에 딸린 부가 카드라 탭 없이 캐러셀에서만 노출(`noTab:true`) + FAQ 아코디언
