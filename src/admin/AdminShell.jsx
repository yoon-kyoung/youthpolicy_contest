...
function NoAccess({ onExit, hasUser }) {
  return (
    <div className="admin-root">
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg)' }}>
        <div style={{ width: '100%', maxWidth: 420, padding: '0 20px' }}>
          <div className="panel-card" style={{ textAlign: 'center' }}>
            <h1 style={{ fontSize: '1.3rem', marginBottom: 6, color: 'var(--title)' }}>관리자 권한이 필요해요</h1>
            <p style={{ fontSize: '0.9rem', color: 'var(--sub)', margin: '12px 0 24px', lineHeight: 1.6 }}>
              {hasUser
                ? '현재 계정은 관리자 권한이 부여되지 않았습니다.\n운영자에게 권한 요청을 부탁드립니다.'
                : '관리자 페이지에 접근하려면 먼저 로그인해 주세요.\n로그인 후 관리자 권한이 부여된 계정이어야 진입할 수 있습니다.'}
            </p>
            <button type="button" onClick={onExit} className="primary-btn" style={{ padding: '12px 24px', fontSize: '0.9rem' }}>
              ← 사이트로 돌아가기
            </button>
          </div>
        </div>
