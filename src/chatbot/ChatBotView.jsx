...
export default function ChatBotView({ bp, favIds, onToggleFav, onGoDetail, resetSignal }) {
  ...
  if (!started) {
    return (
      ...
      >
        <div ref={orb1WrapRef} style={{position:'absolute',left:'29%',top:'24%',width:bp==='mobile'?'58vw':'25.5vw',aspectRatio:'1',pointerEvents:'none',zIndex:0,transition:'transform 0.4s ease-out'}}>
        ...
        <div ref={orb2WrapRef} style={{position:'absolute',left:bp==='mobile'?'36%':'47%',top:'24%',width:bp==='mobile'?'45vw':'19.5vw',aspectRatio:'1',pointerEvents:'none',zIndex:0,transition:'transform 0.4s ease-out'}}>
          ...
          <>
            ...
            <div style={{display:'flex',gap:8}}>
              ...
              <textarea
                ref={promptRef}
                rows={1}
                placeholder={bp==='mobile'?'자유롭게 물어보세요':'자유롭게 물어보세요 (예: 27살 서울 월세 지원)'}
                value={input}
                onChange={(e)=>setInput(e.target.value)}
                onKeyDown={handlePromptKeyDown}
                disabled={loading}
