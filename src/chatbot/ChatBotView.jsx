...
function PrivacyNoticePanel({ bp }) {
  ...
  return (
    <div style={{
      ...
      {/* 접힌 상태: 원형 아이콘 + 하단 문구, 위에 말풍선 안내 */}
      {collapsed && (
        <div style={{ position: 'relative' }}>
          {/* 말풍선 안내 */}
          <div style={{
            position: 'absolute', bottom: '100%', right: -8, marginBottom: 14,
            width: 'max-content', whiteSpace: 'nowrap', padding: '12px 14px', borderRadius: 16,
...
export default function ChatBotView({ bp, favIds, onToggleFav, onGoDetail, resetSignal }) {
  ...
  if (!started) {
    return (
      ...
      >
        <div style={{position:'absolute',left:'24.9%',top:'52%',width:'9vw',aspectRatio:'1',borderRadius:'50%',background:'var(--orb1)',filter:'blur(3.5vw)',pointerEvents:'none',zIndex:0}}/>
        <div style={{position:'absolute',left:'43.2%',top:'46%',width:'6vw',aspectRatio:'1',borderRadius:'50%',background:'var(--orb2)',filter:'blur(2.5vw)',pointerEvents:'none',zIndex:0}}/>
        <div ref={mouseGlowRef} style={{position:'absolute',inset:0,filter:'blur(2vw)',opacity:0.45,pointerEvents:'none',zIndex:0,transition:'background 0.2s ease-out'}}/>
